...
class Client {
    ...
    /**
     * Assign config and input params
     ...
     *
     * @name lib\Client
     * @example client.getKeyValues()
     * @return {*|Promise.<*>}
     */
    getKeyValues (params) {

        params = this._assignParams(params);

        try {
            let data = input.filter({
                keyTableId : 'number,null',
                lang : 'string',
                provider: 'number'
            }, params);
            let carMixin = new mixin.Meta(this);
            return carMixin.getKeyValues(data);
        } catch (err) {
    ...
    getMarkById(params) {
        ...
        try {
            ...
            let carMixin = new mixin.Car(this);
            return carMixin.getMarkById(data);
        } catch (err) {
            throw new exception.ClientError(err.status, err.message);
        }
    }
